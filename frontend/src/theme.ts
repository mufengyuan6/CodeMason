...
function shade(hex: string, amount: number): string {
  ...
  const g = (n >> 8) & 0xff
  const b = n & 0xff
  const mix = (c: number) => Math.round(c + (c < 128 ? amount : -amount))
  const to = (c: number) => Math.max(0, Math.min(255, c)).toString(16).padStart(2, '0')
  return `#${to(mix(r))}${to(mix(g))}${to(mix(b))}`
}

/** 应用主题到 <html>（渲染前调用，防闪烁） */
export function applyTheme(config: ThemeConfig = { id: 'jade-gold', primary: null, accent: null }) {
  const { id = 'jade-gold', primary = null, accent = null } = config
...
export function loadTheme(): ThemeConfig {
  try {
    const saved = JSON.parse(localStorage.getItem(THEME_KEY) || 'null') as ThemeConfig | null
